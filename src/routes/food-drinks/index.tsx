...
export default component$(() => {
  ...
  return (
    <div class='flex flex-col rounded-3xl bg-secondary-color  transition-colors duration-300 dark:bg-slate-400 mr-4 xl:mr-8 w-full'>
        <div class='flex flex-col space-y-2 mt-4 xl:mt-8'>
        ...
        <div class={`${foodStore.createItem? 'hidden' : 'flex'} flex-row space-x-4 justify-between mx-4 xl:mx-8  mt-4`}>
          <button onClick$={()=>foodStore.createItem = true} class='w-1/3 h-10 rounded-xl text-white bg-mid-green'>Create Item</button>
          <button onClick$={()=>foodStore.editItems ? foodStore.editItems = false : foodStore.editItems = true} class='block h-10 rounded-xl text-mid-green w-1/3 border-2  border-mid-green dark:border-secondary-color dark:text-secondary-color bg-secondary-color hover:text-slate-600 hover:bg-quadrary-color dark:hover:text-slate-600 dark:hover:bg-tertiary-color hover:cursor-not-allowed transition-colors duration-300 dark:bg-slate-400'>Edit Items</button>
          {/* edit cart button */}
        ...
        {/* @TODO repropgram create item fucntion */}
        {/* create item diolouge */}
        <div class={` ${foodStore.createItem? 'flex' : 'hidden'} flex-row justify-start m-4 lg:mx-8 h-10`}>
          <input 
            onChange$={(e)=>{setNewItemName(e)}} 
            onFocus$={(e)=>{setNewItemName(e)}} 
            onFocusout$={(e)=>{setNewItemName(e)}} 
            type="text" value={foodStore.newItem.name ? foodStore.newItem.name : 'Name'} class='w-20 md:w-60 mr-4 bg-quadrary-color  transition-colors duration-300 dark:bg-tertiary-color rounded-xl indent-2'
          />
        ...
        <div class='flex flex-row justify-between mx-8 lg:mx-12 my-4' >
          <p>Total Balance</p>
